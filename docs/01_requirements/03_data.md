...
- フェーズ1は**最小MVPとして** Supabase Authでログイン（ログインID＋パスワード）を導入する。メールアドレスは収集・保存せず，IDは内部で合成メールに変換して Supabase Auth に渡す。`role` は `owner` / `staff` の2種類をスキーマで保持するが，実装・運用は owner のみ（staff 個別ログインは後続フェーズ）
