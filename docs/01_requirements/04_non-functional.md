...
Supabase Authによるログイン（ログインID＋パスワード）を使用する。

- **メールアドレスは収集・保存しない**（個人情報を持たない方針）。ログインIDは内部で合成メールに変換して Supabase Auth に渡し，メール送信は行わない
- ログインIDはリクルートからサロンに割り振られるIDを用い，初期パスワードは開発者が発行する。初回ログイン時に本人がパスワードを変更する
- パスワード忘れはセルフサービスで再設定できないため，開発者が初期パスワードを再発行する
- フェーズ1は**最小MVPとして** owner ログインのみ実装する（staff 個別ログインは後続フェーズ）
- 運用はサロンごとに1アカウント（共有）とするが，1サロン内に複数アカウントを持てる設計は維持する
- サインアップ画面・招待画面は提供しない
- ログイン後の遷移先は `/dashboard` に固定する（初回ログイン時のみ `/change-password`）
- パスワードの変更は認証済みユーザー専用の `/change-password` で行う（初回の強制変更＋任意変更。`03_change-password.md` 参照）
- セッション管理はSupabaseが担当
