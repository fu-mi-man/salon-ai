...
`integrations/` は外部サービス（Gemini API 等）との連携仕様。プロンプト設計やAPI呼び出しの設計判断をここに書く。
...
| 対象 | 方針 |
|------|------|
| エクスポートされた関数・クラス・型 | **必須** |
| 非自明なロジックを持つ内部関数 | 推奨 |
| 自明な内部関数（名前で意図が分かるもの） | 不要 |
| 自動生成ファイル（`database.types.ts` 等） | 不要（上書きされるため） |
...
- 説明は日本語で書く
- `@param` / `@returns` は型から自明でない場合のみ付ける
- Claude Code は常に100%を目指す。人間は「エクスポートされたもの」を最低ラインにする
...
フェーズ1は**最小MVPとして** owner ログインだけを実装し（staff 個別ログインは後続フェーズ），招待機能や詳細権限管理は後回しにする。設計の綺麗さより，早く安全に運用できる構成を優先する。

### マイグレーション方針

本番（実データ）投入まではマイグレーションを **CREATE ベースで作り直す**。スキーマ変更が必要なときは ALTER マイグレーションを積まず，既存マイグレーションを直接編集して `supabase db reset` で作り直す（本番前に ALTER が溜まるのを避ける）。

- 対象期間: スキーマ作成〜友人サロンへの引き渡し（実データ投入）まで
- 早期デプロイ（Step2）後も本番にあるのは seed・検証データのみなので作り直してよい
- pg_cron・トリガ等の DDL もこの期間は schema マイグレーションに統合し `db reset` で再現する
- 実データ投入後（引き渡し以降）は forward-only の ALTER マイグレーションに切り替える
