...
- **フェーズ1からSupabase Authを導入する**（ログインID＋パスワード。メールアドレスは収集・保存せず，IDは内部で合成メールに変換して扱う。パスワード忘れは開発者が手動で再発行する。フェーズ1は owner ログインのみ実装し，staff 個別ログインは後続フェーズ。運用はサロン1アカウントだが複数アカウント可の設計は維持）
